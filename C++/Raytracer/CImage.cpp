...
void CImage::plotCalibChart() {
    glm::vec3 rgb1 = {0.0,0.0,0.0};
    glm::vec3 rgb2 = {0.25,0.25,0.25};
    glm::vec3 rgb3 = {0.5,0.5,0.5};
    glm::vec3 rgb4 = {0.75,0.75,0.75};
    glm::vec3 rgb5 = {1.0,1.0,1.0};

    for(int k = 0; k < 5; k++){
        auto curr_height = height/4;
        auto curr_width = width/5;

        for(int j = 10 + (k * 10) + (k * height/6); j < ((height/6) + k * (height/6)); j++) {
            for(int i = 10; i < (width/5); i++ ) {
                switch(k) {
                    case 0:
                        setPixel(i,j, rgb1);
                        break;
